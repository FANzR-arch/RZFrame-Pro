...
- **P0 手动编辑**: 允许用户修改读取到的任何元数据（覆盖原有值）。
- **P1 品牌/Logo**:
    - **Text Mode**: 显示可编辑的品牌文字。
    - **Logo Mode**: 显示品牌 Logo 图片。
    - 支持上传自定义 Logo。
    - Logo 缩放、反色 (Invert)、重置。
...
### v1.1: Core Fixes & Enhancements (当前迭代)
- **F01 [Fix] 字体兼容性修复**: 解决 Windows 下中文字体乱码、显示不全的问题。
- **F02 [New] 时间精度提升**: Exif 时间读取精确到时分秒 (HH:mm:ss)。
- **F03 [Opt] Logo 自由度**: 扩大 Logo 缩放范围 (10%-200%)；切换模板时保留 Logo 设置。
- **F04 [New] 自定义文字颜色**: 支持色盘选择任意颜色。
